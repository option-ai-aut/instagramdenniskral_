...
import { DownloadIcon, SaveIcon, LayoutTemplateIcon, LoaderIcon, CheckIcon } from "lucide-react";
...
export default function CanvasPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      for (let i = 0; i < slides.length; i++) {
        ...
        // We need to export each slide – we'll export the current one, user can iterate
        useCanvasStore.getState().selectSlide(slide.id);
  ...
  return (
    <div className="flex h-full bg-grid">
      ...
      {/* Left – Slides */}
      <div
        className="w-[160px] flex-shrink-0 flex flex-col border-r glass relative z-10"
      ...
      <div className="flex-1 flex flex-col relative z-10">
        {/* Toolbar */}
        <div
          className="px-5 py-3 border-b glass flex items-center gap-3"
          style={{ borderColor: "var(--glass-border)" }}
        >
          <input
            value={carouselTitle}
            ...
            placeholder="Karussell Titel..."
          />

          <button
            onClick={handleExport}
            disabled={exporting}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-white/10 text-xs text-white/50 hover:text-white hover:border-white/20 transition-all disabled:opacity-40"
          >
      ...
      {/* Right – Controls */}
      <div
        className="w-[240px] flex-shrink-0 flex flex-col border-l glass relative z-10"
        style={{ borderColor: "var(--glass-border)" }}
      >
        {/* Tabs */}
        <div
          ...
          {tab === "templates" && (
            <div className="p-4 space-y-2">
              <p className="text-[10px] text-white/30 mb-3">
                Vorlage laden ersetzt alle Slides
              </p>
              {templates.map((tpl) => (
                <button
                  key={tpl.id}
